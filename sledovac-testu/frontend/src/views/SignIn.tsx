...
import { Mail, Lock, AlertCircle, UserPlus, LogIn, User } from 'lucide-react';
      ...
      <div className="w-full max-w-md bg-[#1C1C24]/80 backdrop-blur-md rounded-3xl p-8 border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-brand-textMuted mb-8 text-sm">
            {isLogin ? 'Přihlas se do svého Sledovače testů.' : 'Začni sledovat své studijní výsledky ještě dnes.'}
          </p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {!isLogin && (
              <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-300">
                <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider">Jméno a příjmení</label>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider">Email</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-brand-textMuted">
                  <Mail size={18} />
                </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center">
                <label className="text-xs font-bold text-brand-textMuted uppercase tracking-wider">Heslo</label>
                {isLogin && (
                  <button type="button" className="text-xs text-brand-red hover:text-brand-redHover transition-colors">
                    Zapomenuté heslo?
                  </button>
                )}
              </div>
            ...
            >
              {isLoading ? 'Pracuji...' : (isLogin ? 'Přihlásit se' : 'Zaregistrovat se')}
              {!isLoading && (isLogin ? <LogIn size={18} /> : <UserPlus size={18} />)}
            </button>
          </form>
